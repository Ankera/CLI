...
import { log, Github, Gitee, makeList, getGitPlatform, makeInput, printErrorLog } from '@zm-template/ac-utils';
...
class InstallCommand extends Command {
  ...
  async downloadRepo() {
    const spinner = ora({
      text: chalk.red.bold(`正在下载: ${this.keyword}:${this.selectedTag}中...`),
      spinner: 'material'
    }).start();
    ...
    try {
      ...
      spinner.stop();
      log.success(`\n${chalk.red.bold.underline(this.keyword)}:${this.selectedTag}`, '下载成功');
    } catch (error) {
      printErrorLog(error);
      spinner.stop();
    }
  ...
  async installDependencies() {
    const spinner = ora({
      text: chalk.red.bold(`正在安装依赖: ${this.keyword}:${this.selectedTag}中...`),
      spinner: 'material'
    }).start();

    try {
      const ret = await this.gitAPI.installDependencies(process.cwd() ,this.keyword);
      spinner.stop();
      if(ret){
        log.success(`\n${chalk.red.bold.underline(this.keyword)}:${this.selectedTag}`, '安装成功');
      } else {
        log.error('安装依赖失败...');
      }
    } catch (error) {
  ...
  async doSearch() {
    ...
    if(platform === CK_KEY_GITHUB){
      if(this.mode === SEARCH_MODE_REPO){
        ...
        list = searchResult.items.map((item) => ({
          name: `${chalk.red.bold(item.full_name)} || ${item.description ? chalk.underline.rgb(153, 153, 153)(item.description) : '--'}`,
        ...
        list = searchResult.items.map((item) => ({
          name: `${chalk.red.bold(item.repository.full_name)} || ${item.repository.description ? chalk.underline.rgb(153, 153, 153)(item.repository.description) : '--'}`,
    ...
    if(count > 0) {
      ...
      const keyword = await makeList({
        message: platform === CK_KEY_GITHUB ? `请选择要下载的项目: (共${chalk.red.bold(count)}条数据):` : '请选择要下载的项目',
        choices: list
      })
  ...
  async doSelectTags() {
    const platform = this.gitAPI.getPlatform();
    const params = platform === CK_KEY_GITHUB ? {
      page: this.tagPage,
